...
const { sim, nao, status, activities, mesesN } = require('./util2');
...
module.exports = class Util {
	...
	static diff(timeBase, time2) {
		...
		if (dias) {
			...
			else return `${dias} dias${dias > 1 ? 's' : ''}${minutos ? `e ${minutos} minuto${minutos > 1 ? 's.' : '.'}` : ${horas ? `e ${horas} hora${horas > 1 ? 's.' : '.'`} : '.'}`
		}
		else return `${horas} hora${horas > 1 ? 's' : ''}${minutos ? `${segundos ? ', ' : ' e '} ${minutos} minuto${minutos > 1 ? 's' : ''}` : ''}${segundos ? ` e ${segundos} segundo${segundos > 1 ? 's' : ''}.` : '.'}`
		}
